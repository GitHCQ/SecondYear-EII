...
int main()
{
	...
	/* Calculate the frame and flags for the kernel code, data and bss pages:
		...
		dataFrame, dataFlags
		bssFrame,  bssFlags */
	get_pte((void*)(codePage<<12),&pte); //Direccion virtual de la primera pagina de codigo
	codeFrame = pte>>12;
	codeFlags= pte & 0x00000FFF; //Me quedo con los 12 bits menos significativos
	
	get_pte((void*)(dataPage<<12),&pte); //Direccion virtual de la primera pagina de codigo
	dataPage = pte>>12;
	dataFlags= pte & 0x00000FFF; //Me quedo con los 12 bits menos significativos
	
	get_pte((void*)(bssPage<<12),&pte); //Direccion virtual de la primera pagina de codigo
	bssFrame = pte>>12;
	bssFlags= pte & 0x00000FFF; //Me quedo con los 12 bits menos significativos
